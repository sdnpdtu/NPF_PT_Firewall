...
public class FirewallManager implements PolicyService {
    ...
    public SimpleResponse formalValidation(PolicyRule pr) {
        CopyOnWriteArrayList<String> validconds = new CopyOnWriteArrayList<>(new String[]{"src_ip", "dst_ip","src_mac","dst_mac","src_port","dst_port","tr_prot"});
        ...
        //List<String> supported_conds = Arrays.asList("src_ip", "dst_ip","src_mac","dst_mac","src_port","dst_port","tr_prot");
...
//
        ...
        ////if (!supported_actions.contains(pr.getPolicyActions().get(0).getPolicyVariable()) || !pr.getPolicyActions().get(0).getPolicyValue().equalsIgnoreCase("false"))
        //if (!supported_actions.contains(pr.getPolicyActions().get(0).getPolicyVariable()) )
        //    return new SimpleResponse("Formal error: Incorrect action. Policy action variable must be 'allow'", false);
//
        //if (!pr.getPolicyActions().get(0).getPolicyValue().equalsIgnoreCase("false"))
        //    return new SimpleResponse("Formal error: Firewall policies only support dropping traffic, not allowing it. Thus, the action value must be 'false'.", false);
//
    ...
    // Get firewall rule
    private ForwardingObjective.Builder getFwdObj(CopyOnWriteArrayList<PolicyCondition> pcs, PolicyAction pa, int priority, ApplicationId applicationId){
        ...
        for (PolicyCondition pc:pcs){
            ...
            if (pc.getPolicyVariable().equalsIgnoreCase("device")){
                if (deviceService.getDevice(DeviceId.deviceId(pc.getPolicyValue())) != null){
                    if (device == null) device = DeviceId.deviceId(pc.getPolicyValue());
                }
            }

            // Try to identify (if not already done) the attachment point, from the src_mac field. Also update the traffic selector.
                ...
                if (!hostService.getHostsByMac(MacAddress.valueOf(pc.getPolicyValue())).isEmpty()){
                    if (device == null) device = hostService.getHostsByMac(MacAddress.valueOf(pc.getPolicyValue())).iterator().next().location().deviceId();
                }
                selector.matchEthSrc((MacAddress.valueOf(pc.getPolicyValue())));
            }

            // Try to identify (if not already done) the attachment point, from the src_ip field. Also update the traffic selector.
                ...
                if (!hostService.getHostsByIp(IpAddress.valueOf(pc.getPolicyValue())).isEmpty()){
                    if (device == null)  device = hostService.getHostsByIp(IpAddress.valueOf(pc.getPolicyValue())).iterator().next().location().deviceId();
                }
                selector.matchIPSrc(IpPrefix.valueOf(IpAddress.valueOf(pc.getPolicyValue()),32));
            }

            // Try to identify (if not already done) the attachment point, from the dst_mac field. Also update the traffic selector.
            ...
            // Try to identify (if not already done) the attachment point, from the dst_ip field. Also update the traffic selector.
        ...
        if (protocol.equalsIgnoreCase("tcp")){
            ...
            if (src_port != 0) selector.matchTcpSrc(TpPort.tpPort(src_port));
            if (dst_port != 0) selector.matchTcpDst(TpPort.tpPort(dst_port));
        }
        else if (protocol.equalsIgnoreCase("udp")){
            selector.matchIPProtocol(IPv4.PROTOCOL_UDP);
            if (src_port != 0) selector.matchUdpSrc(TpPort.tpPort(src_port));
            if (dst_port != 0) selector.matchUdpDst(TpPort.tpPort(dst_port));
        }
